...
class GuerreroTest {
	...
	private final Guerrero.Raza RAZA = Guerrero.Raza.HUMANO;
    
    @BeforeEach
    void setUp() {
		try{
        	this.guerrero = new Guerrero(NOMBRE, NIVEL, PTOS_VIDA_MAXIMOS, FUERZA, RAZA);
		}catch(Throwable e){
    ...
    public void testGettersValidos() {
		...
		assertEquals(guerrero.getRaza(), RAZA, "El getter getRaza no devuelve el valor correcto.");
	}
	...
	public void testToString(){
		assertTrue(guerrero.toString().contains("HUMANO"), "El texto no contiene la raza del guerrero.");
	}
